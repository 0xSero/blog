...
export class WatchingEyes {
  ...
  build() {
    ...
    const edgePadding = this.config.edgePadding ?? 14
    // For mobile (tall narrow screens), use max dimension to fill the space
    // For desktop, use min dimension for balanced symmetry
    const isMobileAspect = this.width < this.height * 0.7
    const maxDim = isMobileAspect
      ? Math.max(this.width, this.height)
      : Math.min(this.width, this.height)
    const maxRadius = maxDim / 2 - edgePadding
    // Minimum radius creates clear center space
    const minRadius = Math.max(this.clearRadius * 1.2, 30)

    // Ring count and spacing
    const ringCount = this.config.ringCount || 12
    const eyeScale = this.config.eyeScale || 1
    // Ring spacing - ensures rings don't overlap
    const ringSpacing = this.config.ringSpacing || 45

    this.rings = []
    this.eyes = []

    for (let r = 0; r < ringCount; r++) {
      // Use fixed ring spacing instead of proportional
      const ringRadius = minRadius + r * ringSpacing

      // Skip if ring would be outside bounds
      if (ringRadius > maxRadius) continue

      const ringT = r / Math.max(ringCount - 1, 1)

      // Calculate eye count based on circumference and desired spacing
      const eyeSpacing = this.config.eyeSpacing || 65
      const circumference = TAU * ringRadius
      const rawCount = Math.max(4, Math.floor(circumference / eyeSpacing))
      // Round to nearest even number for bilateral symmetry
      ...
      // Offset alternate rings for interlocking pattern
      ...
      for (let e = 0; e < eyeCount; e++) {
        ...
        // Deterministic phase based on position (no randomness for symmetry)
        ...
        const eye = {
          ...
          phase,
          ringT,
          // Deterministic blink timing for symmetric animation
